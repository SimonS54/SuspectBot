...
class Error(commands.Cog):
    ...
    # Command to provide fixes for Marvel Rivals Predator error codes
    @app_commands.command(name="marvelprederror", description="Fixes for common Marvel Rivals Predator error codes")
    async def marvelprederror(self, interaction: discord.Interaction):
        user_roles = [role.id for role in interaction.user.roles]
        if any(role_id in ALLOWED_ROLE_IDS + VERIFIED_CUSTOMER_ROLE_ID for role_id in user_roles):
            embed = discord.Embed(
                title="🔧 Marvel Rivals Predator Error Fixes",
    ...
    # Command to provide fixes for CS2 Predator error codes
    @app_commands.command(name="cs2prederror", description="Fixes for common CS2 Predator error codes")
    async def cs2prederror(self, interaction: discord.Interaction):
        user_roles = [role.id for role in interaction.user.roles]
        if any(role_id in ALLOWED_ROLE_IDS + VERIFIED_CUSTOMER_ROLE_ID for role_id in user_roles):
            embed = discord.Embed(
                title="🔧 CS2 Predator Error Fixes",
                description="No common errors reported—follow the setup guide or contact support if issues persist.",
                color=discord.Color.red()
            )
            embed.set_footer(text="Powered by SuspectServices • Error Section", icon_url=self.bot.user.avatar.url)
